...
(function () {
    ...
    var tmp = window.location.pathname,
            URL =  window.UEDITOR_HOME_URL || '/statics/ueditor/';//window.UEDITOR_HOME_URL||tmp.substr(0,tmp.lastIndexOf("\/")+1).replace("_examples/","").replace("website/","");//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
    UEDITOR_CONFIG = {
        imagePath: window.UEDITOR_IMG_URL || "/", //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
        ...
        filePath: window.UEDITOR_FILE_URL ||  "/",  //附件文件夹保存路径
	    catchRemoteImageEnable:true,                                   //是否开启远程图片抓取
        catcherUrl:URL + "server/submit/getRemoteImage.php",             //处理远程图片抓取的地址
        localDomain: window.DOMAIN || "www.huisw.com",                                        //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地
	    imageManagerPath:URL + "server/submit/imageManager.php",       //图片在线浏览的处理地址
        ...
        //锚点的样式,注意这里背景图的路径
        '.anchorclass{border: 1px dotted #0000FF;cursor: auto;display: inline-block;height: 16px;width: 15px;}' +
        //设置四周的留边
        ...
        snapscreenHost: window.DOMAIN || '127.0.0.1', //屏幕截图的server端文件所在的网站地址或者ip，请不要加http://
