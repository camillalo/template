...
//$weixin = new class_weixin_adv("wx5fdc8233e9678fdf", "ecfbc6a335c7856e3e331330ba15cd57");  //实类化
...
/*$members_list = $weixin->get_user_list();
$count=count($members_list['data']['openid']);
echo $count;
for($i=0;$i<$count;$i++){ 
    $openid = $members_list['data']['openid'][$i];
...
class class_weixin_adv
{
    ...
    public function __construct($appid = NULL, $appsecret = NULL)
    {
        ...
        public function responseMsg($replays_data){
        ...
        if (!empty($postStr)){
                ...
                if(!empty( $keyword ))
                {
                    ...
                    // if($keyword=="1"){
                    // $contentStr = "联系方式：汕头市金平区华乾大厦703,1341702551，           email：30090032@qq.com";}
                    // if($keyword=="2"){
                    // $contentStr = "最新资讯：《汕头市环境违法行为举报奖励办法》将于8月1日实施。";}
                    // if($keyword=="3"){
                    // $contentStr = "31离婚协议\n32租房合同\n33交通事故协议书";} 

                    if(!empty($replays_data)){ 
                        foreach ($replays_data as $data) { 
